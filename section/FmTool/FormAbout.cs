...
namespace section
{
    public partial class FormAbout : Form
    {
        ...
        {
            #region 程序版本号读取
            string v1 = "程序集版本：" + System.Reflection.Assembly.GetExecutingAssembly().GetName().Version.ToString() + "\n";
            string v2 = "文件版本：" + Application.ProductVersion.ToString() + "\n";
            //string v3 = "部署版本：" + System.Deployment.Application.ApplicationDeployment.CurrentDeployment.CurrentVersion.ToString();
            #endregion
            #region 免责声明
            string Disclaimer_Notices =Environment.NewLine + 
    ...
    4. 用户使用须知
        4.1 本软件提供各类市政管线及其附属构筑物的工程量计算功能。
        4.2 本软件仅适用于Windows 7/10操作系统；其他Windows系统未经测试，无法保证软件正常运行；本软件不适用于所有非Windows系统。如果用户在安装本软件后因任何原因欲放弃使用，可删除本软件。
        4.3 本软件由作者提供产品支持。
        ...
        4.8 隐私权保护：为了更好地改进软件和服务，在用户关闭本软件时，本软件会向作者报告软件所在计算机的唯一标识，具体报告方法为发送一封含Log附件的邮件，此邮件将被用于记录软件操作以便重现可能出现的程序崩溃现象，帮助更好的改善程序。作者不会将此数据与用户的个人身份信息相关联。
    5.免责与责任限制
        5.1 本软件经过详细的测试，但不能保证与所有的软硬件系统完全兼容，不能保证本软件完全没有错误。如果出现不兼容及软件错误的情况，用户可将情况报告作者，获得技术支持。如果无法解决兼容性问题，用户可以删除本软件。
    ...
    7. 其他条款
        ...
        7.3 本协议的一切解释权与修改权归作者";
            #endregion
            LBLversion.Text = "程序版本号: " + System.Reflection.Assembly.GetExecutingAssembly().GetName().Version.ToString() + "  ";
            LBLversion.Text += mscCtrl.DC == null ? "" : "文件版本号: " + mscCtrl.DC.fileVersion;
            TXTdn.Text = Disclaimer_Notices;
            TXTdn.Select(0, 0);
        }
