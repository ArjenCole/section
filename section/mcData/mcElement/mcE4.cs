...
{
    [Serializable]//序列化类
    public class mcE4 : mcElement//顶管
    {
        public mcE4()
        {
            unit = "m";
        }
        public mcE4(XElement pXE)
        {
            readXE(pXE);
            unit = "m";
        }
        public mcE4(object[] pData, Dictionary<string, double> pDicPE, Dictionary<string, double> pDicPF)
        {
        ...
        public override List<KeyValuePair<string, Rectangle>> OntologyShape()
        {
            ...
            Rectangle outsideEllipse = new Rectangle(-Convert.ToInt16(mList.First().Dn / 2.0 + At.t), 0, Convert.ToInt16(mList.First().Dn + At.t * 2), Convert.ToInt16(mList.First().Dn + At.t * 2));
            rtList.Add(new KeyValuePair<string, Rectangle>("Ellipse", outsideEllipse));
            Rectangle insideEllipse = new Rectangle(-Convert.ToInt16(mList.First().Dn / 2.0), Convert.ToInt16(At.t), Convert.ToInt16(mList.First().Dn), Convert.ToInt16(mList.First().Dn));
        ...
        protected override Dictionary<string, mcQ> getDQ()
        {
            ...
            rtD = mscGroove.PlusDic_StrmcQ(rtD, cal_pipes("顶管"));//加上管材
